...
function OasisCarousel() {
  ...
  return (
    <>
      <div style={{ position: 'relative', backgroundColor: '#EEEEEE', overflow: 'hidden' }}>
        ...
        >
          {oasisScreenshots.map((shot, i) => (
            <div
              key={i}
              style={{ minWidth: '100%', flexShrink: 0, cursor: 'zoom-in', position: 'relative', height: '420px' }}
      ...
      {/* Lightbox */}
      {lightboxOpen && typeof window !== 'undefined' && ReactDOM.createPortal(
        <div
        ...
        >
          ...
          <div
            ...
            style={{ position: 'relative', maxWidth: '100%', maxHeight: '90vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          >
            <Image
...
function NCBDesignSystemModal({ onClose }: {onClose: () => void;}) {
  ...
  const modalContent =
  <div
    style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.72)', padding: '8px' }}
    onClick={onClose}
    aria-modal="true"
    role="dialog"
