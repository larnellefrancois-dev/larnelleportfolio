...
function Block({ block }: { block: ContentBlock }) {
  return (
    <MotionReveal as="section" style={{ marginBottom: 'var(--space-xl)' }}>
      {block.heading && (
        <h2 style={{ fontSize: 'var(--step-2)', marginBottom: 'var(--space-sm)' }}>
          {block.heading}
        </h2>
      )}
...
export default function PageTemplate({
  ...
  return (
    <article
      className="ds-container ds-container--narrow"
    ...
    >
      ...
      <h1
        style={{
          ...
          marginBottom: subtitle ? 'var(--space-sm)' : 'var(--space-lg)',
        }}
      >
        {title}
      </h1>
      {subtitle && (
        <p className="ds-lede" style={{ marginBottom: 'var(--space-lg)' }}>
          {subtitle}
        </p>
      )}
        ...
        >
          {navLinks.map((l) => (
            <Link
              ...
              style={{ marginLeft: l.direction === 'next' ? 'auto' : undefined }}
            >
              {l.direction === 'prev' && <span aria-hidden="true">←</span>}
              {l.label}
              {l.direction === 'next' && <span aria-hidden="true">→</span>}
            </Link>
          ))}
        </nav>
      )}
